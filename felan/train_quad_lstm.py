...
def train_step(state, x_batch, y_batch, norm_tau, apply_fn):
    def loss_fn(params):
        ...
        # per-Dim normalisierter MSE, dann Mean
...
def train_lstm(state, q_flat, qd_flat, tau_flat, valid_starts,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % log_period == 0 and epoch > 0:
            ...
            print(f"epoch {epoch:5d}: mean loss (last {log_period}) = {recent:.6f}")
...
if __name__ == "__main__":
    ...
    model = LSTMBlackBox(n_dof=nq, config=lstm_cfg, z_dim=6)   # n_dof nur der Kompatibilität wegen
