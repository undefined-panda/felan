...
def get_config_from_dict(kwargs):
    ...
    if skew_sym_ineq:
        # Skew-symmetric matrix input + LR
...
class FeLaN(nn.Module):
    ...
    def get_base_rot_nn_output(self, q):
        ...
        S_input, LR_input = split_by_lengths(self.lbase_rot_net(q),
                                             output_lengths, 
                                             axis=1)
        ...
        LR = self.vmap_get_L_from_output(LR_input, self.lbase_rot_net.config)
        
        LR_diff = jnp.diagonal(LR, axis1=1, axis2=2) - LR_input[:,:3] - self.lbase_rot_net.config.epsilon
        return S, LR, LR_diff
    ...
    def get_base_nn_output(self, q):
        ...
        output_lengths = [first_output_length,
                          self.lbase_rot_net.config.l_output_size,
                          self.lbase_rot_net.config.l_output_size]
        S_input, LL_input, LR_input = split_by_lengths(self.lbase_rot_net(q),
                                             output_lengths,
                                             axis=1)
        ...
        LR = self.vmap_get_L_from_output(LR_input, self.lbase_rot_net.config)
        LL = self.vmap_get_L_from_output(LL_input, self.config.base_mass_config)

        LR_diff = jnp.diagonal(LR, axis1=1, axis2=2) - LR_input[:,:3] - self.lbase_rot_net.config.epsilon
        return S, LL, LR, LR_diff
    ...
    def get_rot_inertia_from_tri_ineq_sigma(self, Sigma, W):
        ...
        LR_pen = LR_prod_shift

        return LR_prod_new, HR, LR_pen, LR_prod_eigvalues
    
    def get_linear_factor_from_mass(self, U, raw_mass):
        U_prod = jnp.transpose(U, (1, 0)) @ U

        U_prod_eig = jnp.real(jnp.linalg.eigvalsh(U_prod))
        U_prod_max_eigvalue = jax.lax.stop_gradient(jnp.max(U_prod_eig))

        mass = apply_act_fn(raw_mass - U_prod_max_eigvalue, 'Softplus', self.config.diff_mass_softplus_beta) + self.config.mass_epsilon + U_prod_max_eigvalue
       
        LL_input = mass * jnp.eye(3) - U_prod
    ...
    def get_inertia_matrix_humanoid(self, q_full):
        ...
        if self.config.mass_ineq:
            ...
            S, Sigma_LR, LR_diff = self.get_base_rot_nn_output(q)
            Sigma_mat = jnp.transpose(Sigma_LR, (0, 2, 1)) @ Sigma_LR + self.config.final_sigma_epsilon * jnp.eye(Sigma_LR.shape[-1])

        else:
            S, LL, Sigma_LR, LR_diff = self.get_base_nn_output(q)
            Sigma_mat = jnp.transpose(Sigma_LR, (0, 2, 1)) @ Sigma_LR + self.config.final_sigma_epsilon * jnp.eye(Sigma_LR.shape[-1])
        ...
        if self.config.tri_ineq:
            LR_prod_new, HR, LR_pen, LR_prod_eigvalues = self.vmap_get_rot_inertia_from_tri_ineq_sigma(Sigma_mat, W)
            LR = reordered_cholesky(LR_prod_new)

        else:        
            # Default case
            LR = Sigma_LR
            LR_pen = jnp.zeros((n_batch, 1))

        ## Skew-symmetric constraint
        if self.config.skew_sym_ineq:
            LRL_prod_term = jnp.transpose(K, (0, 2, 1)) @ W

            LRL_input = jnp.transpose(S, (0, 2, 1)) - LRL_prod_term
            LRL = jax.scipy.linalg.solve_triangular(jnp.transpose(LR, (0, 2, 1)), jnp.transpose(LRL_input, (0, 2, 1)), lower=False)
        ...
        #### LL ####
        if self.config.mass_ineq:
            LL_input, mass, U_prod_eig = self.vmap_get_linear_factor_from_mass(U, raw_mass[...,None])
            diff_mass = jnp.sum(jax.nn.softplus(-(raw_mass - U_prod_eig) + self.config.diff_mass_shift_loss))
            LL = reordered_cholesky(LL_input)
        ...
        extras = {
            ...
            'diff_rot_eigenvalue': LR_pen.squeeze(),
        }
